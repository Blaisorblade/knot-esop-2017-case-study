(* This file can be used as a general framework for environments.
   We could simplify by simply focusing on natural numbers and lists,
   but it would cost time. *)
...
Section Properties.
  ...
  Lemma app_assoc_map_push : 
    x ~ (g a) ++ (map g E ++ F) = (map g (x ~ a ++ E)) ++ F.
  Proof.
    auto.
  Qed.

  Lemma eq_push_inv : 
    x ~ a  ++ E = y ~ b ++ F -> E = F /\ x = y /\ a = b.
  Proof.
    simpl. intro H. inversion H. split; try reflexivity; split; reflexivity.
  Qed.
...
Implicit Arguments eq_push_inv [A x y a b E F].

Hint Rewrite 
  map_nil 
  map_app : rew_env.

Hint Rewrite <- app_assoc : rew_env.
...
Section BindsProperties.
  ...
  Lemma binds_cons_4 :
    binds x a (x ~ a ++ E).
  Proof. unfold binds; intros; simpl; tauto. Qed.
  ...
  Lemma binds_head : 
    binds x a (x ~ a ++ E).
  Proof.
    intros. unfold binds. simpl. left; auto.
  Qed.
...
Hint Resolve
  @binds_one_3 @binds_cons_2 @binds_cons_3 @binds_cons_4
  @binds_app_2 @binds_app_3
  @binds_map_2 @binds_head.
...
Section UniqDerived.
  ...
  Lemma uniq_map_app_1 : forall (f : A -> A),
    uniq (F ++ E) ->
    uniq (map f F ++ E).
  Proof. clear. solve_uniq. Qed.

  Lemma uniq_map_app_2 : forall (f : A -> A),
    uniq (E ++ F) ->
    uniq (E ++ map f F).
  Proof. clear. solve_uniq. Qed.
  ...
  Lemma uniq_dom_only :
    uniq (E ++ x ~ a ++ F) -> uniq (E ++ x ~ b ++ F).
  Proof. intros; solve_uniq. Qed.

  Lemma uniq_dom_only_1 : 
    uniq (x ~ a ++ E) -> uniq (x ~ b ++ E).
  Proof. intros; solve_uniq. Qed.
...
Hint Resolve @uniq_insert_mid @uniq_map_app_1 @uniq_map_app_2.
